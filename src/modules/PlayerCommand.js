...
const commands = [
    new Command("help", "lists available commands", "[cmd]", UserRoleEnum.GUEST, (player, args) => {
        let cmd = args[1];
    ...
    new Command("login", "opcode", "<password>", UserRoleEnum.GUEST, (player, args) => {
        let pass = args[1];
        if (!pass || !(pass = pass.trim()))
            return send(player, "Login Failed");
        let user = player.server.userList.find(c => c.password == pass && (c.ip == player.socket.remoteAddress || c.ip == "*"));
        if (!user) return send(player, "Login Failed");
        ...
        send(player, `Logged in as: ${user.name}`);
    }),
    new Command("logout", "opcode", "", UserRoleEnum.GUEST, (player, args) => {
        if (player.userRole == UserRoleEnum.GUEST)
            return send(player, "Failed");
    ...
    new Command("mass", "adds mass", "<mass> [id]", UserRoleEnum.MODER, (player, args) => {
        const mass = parseInt(args[1]);
        if (isNaN(mass)) return send(player, "Mass string missing");
        ...
        if (!p) return send(player, "Didnt find player with id " + id);
    ...
    new Command("add-minion", "adds minions", "<count> [id] [name]", UserRoleEnum.MODER, (player, args) => {
    ...
    new Command("rm-minion", "removes minions", "[id]", UserRoleEnum.MODER, (player, args) => {
    ...
    new Command("spawnmass", "change your spawnmass", "<mass> [id]", UserRoleEnum.ADMIN, (player, args) => {
        const mass = parseInt(args[1]);
        if (isNaN(mass))
...
class PlayerCommand {
    ...
    processMessage(from, msg) {
        ...
        if (cmd)
            ...
            else send(from, "Failed - Access unavailable");
        else send(from, "Invalid command, use /help for available commands");
    }
}
