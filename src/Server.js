...
// }
    ...
    sendTimer(clients, e) {
        for (var i = 0, len = clients.length; i < len; i++) {
            if (clients[i]) {
                clients[i].client.sendPacket(new Packet.UpdateTimer(e));
            }
        }
    }
    ...
    async isResidentialIp(ip) {
        // Replace with your preferred IP info service URL or API
        const apiUrl = `https://ipinfo.io/${ip}/json`;
        try {
            const response = await axios.get(apiUrl);
            // Assuming the service returns a "type" or "org" field that indicates if the IP is residential
            const ipType = response.data.org || response.data.type;

            // You can adjust this based on the IP service response; for example:
            if (ipType && ipType.includes("Residential")) {
                return true; // The IP is residential
            }
        } catch (error) {
            console.error("Error checking IP type:", error);
        }

        return false; // Default to non-residential or unable to verify
    }
    ...
    onClientSocketOpen(ws, req) {
        ...
        // Allow localhost (127.0.0.1) to always pass through
        if (ws._socket.remoteAddress === '127.0.0.1') {
            // Proceed to setup client if IP is localhost
            ws.isConnected = true;
            ws.remoteAddress = ws._socket.remoteAddress;
            ws.remotePort = ws._socket.remotePort;
            ws.lastAliveTime = Date.now();

            Logger.write("CONNECTED (WHITELISTED) " + ws.remoteAddress + ":" + ws.remotePort + ", origin: \"" + req.headers.origin + "\"");

            ws.player = new Player(this, ws);
            ws.client = new Client(this, ws);
            ws.auth = new AuthHandler(this, ws);
            ...
            ws.on('message', message => {
                ...
                ws.auth.message(message);
                ws.client.message(message);
            });
            ...
            ws.on('close', reason => {
                ...
                Logger.write("DISCONNECTED (WHITELISTED) " + ws.remoteAddress + ":" + ws.remotePort + ", code: " + ws._closeCode +
                    ", reason: \"" + ws._closeMessage + "\", name: \"" + ws.player._name + "\"");
            });

            // Increment socket count and add the client to the list
            this.socketCount++;
            this.clients.push(ws);

            // Check for external minions (whatever that is in your application)
        ...
        const connectionHeader = req.headers['connection'] || 'Unknown';
        const upgradeHeader = req.headers['upgrade'] || 'Unknown';
        const xForwardedFor = req.headers['x-forwarded-for'] || '';
        const xRealIp = req.headers['x-real-ip'] || '';
        const via = req.headers['via'] || '';
        const forwarded = req.headers['forwarded'] || '';

        // Add logic to detect proxies based on header analysis
        if (xForwardedFor || xRealIp || via || forwarded) {
            ws.close(1000, "proxy detected");
            return;
        }

        if (!userAgent || userAgent === 'Unknown' || userAgent.length < 10) {
            ws.close(1000, "Invalid User-Agent");
            return;
        }

        if (!acceptLanguage || acceptLanguage === 'Unknown') {
            ws.close(1000, "Invalid Accept-Language");
            return;
        }

        if (connectionHeader.toLowerCase() !== 'upgrade' || upgradeHeader.toLowerCase() !== 'websocket') {
            ws.close(1000, "Invalid WebSocket headers");
            return;
        }

        // Existing checks (token, IP ban, etc.)
        const urlParams = new URLSearchParams(req.url.split('?')[1]);
        const randomToken = urlParams.get('rtk1');

        if (!randomToken || randomToken.length !== 64) {
            ws.close(1000, "token error");
            return;
        }

        // IP checks, connection limits, etc.
        ...
        // Check if the IP is residential
        this.isResidentialIp(ws._socket.remoteAddress).then(isResidential => {
            if (!isResidential) {
                ws.close(1000, "Non-residential IP detected");
                return;
            }

            // Proceed to setup client if everything checks out
            ws.isConnected = true;
            ws.remoteAddress = ws._socket.remoteAddress;
            ws.remotePort = ws._socket.remotePort;
            ws.lastAliveTime = Date.now();
            ...
            // If there was an error checking the IP type, handle it here
            ws.close(1000, "Error checking IP type");
            Logger.error("Error checking IP type for: " + ws.remoteAddress);
        });
    }
    ...
    sendMsg(e, f) {
        for (var i = 0, len = this.clients.length; i < len; i++) {
            if (this.clients[i]) {
                this.clients[i].client.sendPacket(new Packet.sendMsg(e, f));
            }
        }
    }
    ...
    movePlayer(cell, client) {
        ...
        // update remerge
        var time = this.config.playerRecombineTime, base = Math.max(time, cell.radius * 0.2) * 2;
    ...
    boostCell(cell) {
        ...
        var speed;
        if (cell.type === 2) {
            speed = cell.boostDistance / 1; // val: 87
        } else {
            speed = cell.boostDistance / 6.5; // val: 87
        }
        cell.boostDistance -= speed * 0.55; // decays from speed
    ...
    resolveCollision(m) {
        ...
        const OVERLAP = this.config.eatOverlap || 0.80;   // 0.75-0.85 recomendado
        if (m.d >= check.radius - cell.radius * OVERLAP) return; // demasiado lejos
    ...
    splitPlayerCell(client, parent, angle, mass, type) {
        ...
        if (type === 2) {
            newCell.setBoost(this.config.splitVelocity / 2.25 * Math.pow(size, 0.0822), angle);
        } else {
            newCell.setBoost(this.config.splitVelocity * Math.pow(size, 0.0822), angle);
        }
    ...
    loadFiles() {
        ...
        // Load bad words
        var fileNameBadWords = './other/badwords.txt';
        try {
        ...
        // Load ip ban list
        var fileNameIpBan = './other/ipbanlist.txt';
        try {
